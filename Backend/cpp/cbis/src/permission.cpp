...
void Permission::count(
  ...
  std::string stmt = "SELECT COUNT(id) FROM permissions";
...
void Permission::info(const HttpRequestPtr &req,
  ...
  async_run([=]() -> Task<> {
    try {
      ...
      if (r.size() > 0) {
        ...
        info["access"] = row["access"].as<Json::Value>();
...
void Permission::list(
  ...
  async_run([=]() -> Task<> {
    try {
      std::string stmt = "SELECT * FROM permissions";
