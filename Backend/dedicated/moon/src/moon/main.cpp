...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (file::read_all(bootstrap, std::ios::in).find("_G[\"__init__\"]") !=
        std::string::npos) {
      ...
      enable_stdout =
          lua_opt_field<bool>(L, -1, "enable_stdout", enable_stdout);
    ...
    if (lua_search_path.find("lualib/?.lua") == std::string::npos) {
      // By default, lualib and service directories are added to the lua search
      // path
      auto search_path = fs::absolute(fs::current_path());
      if (!fs::exists(search_path / "lualib"))
        search_path = fs::absolute(directory::module_path());
      MOON_CHECK(fs::exists(search_path / "lualib"),
                 "can not find moon lualib path.");
      auto strpath = search_path.string();
      moon::replace(strpath, "\\", "/");
      if (!lua_search_path.empty() && lua_search_path.back() != ';')
        lua_search_path.append(";");
      lua_search_path.append(moon::format("%s/lualib/?.lua;%s/service/?.lua;",
                                          strpath.data(), strpath.data()));
    }

    server_->set_env("PATH", moon::format("package.path='%s;'..package.path;",
                                          lua_search_path.data()));
    ...
    if (!server_->get_env("CPATH")) {
      ...
      if (fs::exists(search_path / "clib")) {
        ...
        server_->set_env(
            "CPATH", moon::format("package.cpath='%s/clib/%s;'..package.cpath;",
                                  strpath.data(), ext->data(), strpath.data()));
      }
    }
    ...
    // Change the working directory to the directory where the opened file is
    // located.
...
extern "C" {
void MOON_EXPORT send_message(uint8_t type, uint32_t receiver, int64_t session,
                              const char *data, size_t len) {
  auto svr = wk_server.lock();
  if (nullptr == svr)
    return;
  svr->send_message(
      moon::message{type, 0, receiver, session, std::string_view(data, len)});
}

void MOON_EXPORT send_integer_message(uint8_t type, uint32_t receiver,
                                      int64_t session, ssize_t val) {
  auto svr = wk_server.lock();
  if (nullptr == svr)
    return;
