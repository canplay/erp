<template>
  <div class="cultivate">
    <div class="boss">
      <div class="boss-box">
        <span class="el-tag el-tag--warning" @click="openBossInfo">{{
          boss.name
        }}</span>
        <el-alert
          class="desc"
  ...
  methods: {
    ...
    fightBoss() {
      if (this.player.level < this.$maxLv) {
        ...
        this.stopFightBoss();
        this.texts.push(
          `你的境界尚未达到${this.$levelNames(this.$maxLv)}, ${
            this.boss.name
          }对于你的挑战不屑一顾`
        );
        return;
      ...
      if (this.guashaRounds > 1) {
        ...
        if (this.boss.health <= 0) {
          ...
          this.texts.push(
            `你击败${this.boss.name}后，获得了<span class="el-tag el-tag--${
              equipItem.quality
          ...
          this.texts.push(
            isPlayerHit
              ? `你攻击了${this.boss.name}，${
                  isCritical ? "触发暴击" : ""
                }造成了${playerHarm}点伤害，剩余${this.boss.health}气血。`
              : `你攻击了${this.boss.name}，对方闪避了你的攻击，你未造成伤害，剩余${this.boss.health}气血。 `
          );
          this.texts.push(
            isBHit
              ? `${this.boss.name}攻击了你，${
              ...
              : `${this.boss.name}攻击了你，你闪避了对方的攻击，对方未造成伤害，你剩余${this.player.health}气血。 `
          );
        }
      } else {
        // 恢复默认回合数
    ...
    getMinuteDifference(specifiedTimestamp) {
      ...
      specifiedTimestamp =
        specifiedTimestamp == 0 ? currentTimestamp : specifiedTimestamp;
      // 计算时间差（分钟数）
      const timeDifferenceInSeconds = Math.abs(
        currentTimestamp - specifiedTimestamp
      );
