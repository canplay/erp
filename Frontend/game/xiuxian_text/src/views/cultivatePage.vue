    ...
    <div class="cultivation-info">
      <div class="realm-display">
        当前境界：<span
          class="realm-text"
          v-text="
            `${$levelNames(player.level)}(${player.reincarnation || 0}转)`
          "
        />
      </div>
      <el-progress
      ...
      const randomEvents = [
        {
          ...
          name: "灵石",
          amount: 100,
          description: "你发现了一堆灵石！",
        },
        {
          type: "cultivation",
          name: "顿悟",
          amount: 500,
          description: "你突然顿悟，修为大涨！",
        },
        {
          type: "item",
          name: "丹药",
          effect: "增加100点修为",
          description: "你获得了一颗珍贵的丹药！",
        },
        {
          type: "skill",
          name: "剑法",
          effect: "增加10%攻击力",
          description: "你领悟了一门高深剑法！",
        },
        {
          type: "lucky",
          name: "雷劫",
    ...
    reincarnationBreakthrough() {
      ...
      if (this.player.level == this.$maxLv) {
        if (this.player.points) {
          this.$notifys({
            title: "未满足转生条件",
            message: `当前还有${this.player.points}境界点未使用, 无法转生`,
          });
          return;
        }
        ...
        this.$notifys({
          title: "未满足转生条件",
          message: `境界需要达到<span class="textColor">${this.$levelNames(
            this.$maxLv
          )}</span>才能满足转生条件`,
          dangerouslyUseHTMLString: true,
        });
