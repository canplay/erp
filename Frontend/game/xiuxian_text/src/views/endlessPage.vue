  ...
  <div class="endless-tower">
    <div class="battle-info">
      ...
      <div class="monster-info" v-if="monster">
        <tag class="name" type="danger" @click="openInfo('monster')">{{
          monster.name
        }}</tag>
        <el-progress
...
export default {
  ...
  created() {
    ...
    // 当前层数
    this.currentFloor =
      this.player.highestTowerFloor > 1 ? this.player.highestTowerFloor - 1 : 1;
  },
  mounted() {
    // 生成日志
    this.battleLogs.push(
      `欢迎来到无尽塔, 这里是无尽塔的第${this.currentFloor}层, 你的爬塔最高记录为${this.player.highestTowerFloor}层`
    );
    // 生成怪物
  ...
  computed: {
    ...
    sweepData() {
      return [
        ...
        {
          name: "获得修为",
          suffix: `${this.$formatNumberToChineseUnit(
            this.sweepResults.expGain
          )}点`,
        },
        {
          name: "获得灵石",
          suffix: `${this.$formatNumberToChineseUnit(
            this.sweepResults.moneyGain
          )}块`,
        },
        {
          name: "获得装备",
    ...
    buttonData() {
      return [
        {
          text: this.isAutoFighting ? "停止对战" : "自动对战",
          click: () => this.toggleAutoFight(),
          disabled: this.isSweeping || this.player.health <= 0,
        },
        {
          text: "进行对战",
          click: () => this.fight(),
        ...
        {
          text: this.isSweeping ? "停止扫荡" : "开始扫荡",
          click: () => this.toggleSweep(),
          disabled: this.isAutoFighting || this.player.health <= 0,
        },
  ...
  methods: {
    ...
    fight() {
      ...
      // 玩家攻击怪物
      const playerAttackResult = combatSystem.executeCombatRound(
        this.player,
        this.monster
      );
      this.generateCombatLog(
        this.player.name,
        this.monster.name,
        playerAttackResult
      );
      ...
      // 怪物攻击玩家
      const monsterAttackResult = combatSystem.executeCombatRound(
        this.monster,
        this.player
      );
      this.generateCombatLog(
    ...
    handleMonsterDefeat() {
      ...
      if (
        this.currentFloor % 5 === 0 &&
        !this.player.rewardedTowerFloors.includes(this.currentFloor)
      ) {
        this.player.props.cultivateDan += 500;
        this.player.rewardedTowerFloors.push(this.currentFloor);
        this.battleLogs.push(
          `恭喜你通过第 ${this.currentFloor} 层，获得额外奖励：500培养丹！`
        );
      }
      // 如果当前层数大于最高层数
      ...
      // 日志
      this.battleLogs.push(
        `成功通过第 ${this.currentFloor - 1} 层，自动前往第 ${
          this.currentFloor
        } 层`
      );
      // 生成新的怪物（下一层）
    ...
    handlePlayerDefeat() {
      ...
      this.battleLogs.push(
        `${this.monster.name}: ${boss.drawPrize(this.monster.level).text}`
      );
      // 关闭自动战斗
    ...
    getRandomEquipment() {
      ...
      this.battleLogs.push(
        `你发现了一个宝箱，打开后获得${this.$levels[equipItem.quality]}${
          equipItem.name
    ...
    sweep() {
      ...
      if (this.sweepTime % 60 === 0)
        this.battleLogs.push(
          `扫荡结果：目前已扫荡${this.formatTime(
            this.sweepTime
          )}，恭喜你获得了${this.sweepResults.expGain}点修为，${
            this.sweepResults.moneyGain
          }灵石和${this.sweepResults.equipmentGained}件装备。`
        );
    },
    // 扫荡战斗
    sweepFight() {
      ...
      // 日志
      this.battleLogs.push(
        `扫荡结果：恭喜你获得了${expGain}点修为，${moneyGain}块灵石${
          equipmentGained ? "和1件装备" : "。"
        }`
      );
    },
    setupObserver() {
