  ...
  methods: {
    ...
    operate(oprName) {
      ...
      switch (oprName) {
        ...
        case "e":
        case "harvestPet":
          if (!this.isEnd && !this.isCaptureFailed)
            this.harvestPet(this.monster);
          break;
        // 立马撤退
        case "r":
        case "runAway":
      ...
      this.$alert("", this.monster.name, {
        ...
        message: `<div class="monsterinfo">
                        <div class="monsterinfo-box">
                            <p>境界: ${
                              this.player.level == 0
                                ? this.$levelNames(this.player.level + 1)
                                : this.$levelNames(this.player.level)
                            }</p>
                            ...
                            <p>闪避率: ${
                              this.monster.dodge > 0
                                ? this.monster.dodge * 100 > 100
                                  ? 100
                                  : (this.monster.dodge * 100).toFixed(2)
                                : 0
                            }%</p>
                            <p>暴击率: ${
                              this.monster.critical > 0
                                ? this.monster.critical * 100 > 100
                                  ? 100
                                  : (this.monster.critical * 100).toFixed(2)
                                : 0
                            }%</p>
                            <p>收服率: ${successRate}%</p>
                        </div>
    ...
    fightMonster() {
      ...
      if (this.guashaRounds > 1) {
        ...
        if (this.monster.health <= 0) {
          ...
          // 发送提示
          this.texts.push(
            `击败${this.monster.name}后你获得了${reincarnation}颗培养丹`
          );
          ...
          this.texts.push(
            !isPHit
              ? `你攻击了${this.monster.name}，对方闪避了你的攻击，你未造成伤害，剩余${this.monster.health}气血。`
              : `你攻击了${this.monster.name}，${
                  isCritical ? "触发暴击" : ""
                }造成了${playerHarm}点伤害，剩余${this.monster.health}气血。`
          );
          // 野怪
          this.texts.push(
            !isMHit
              ? `${this.monster.name}攻击了你，你闪避了对方的攻击，对方未造成伤害。`
              : `${this.monster.name}攻击了你，${
                  isMCritical ? "触发暴击" : ""
                }造成了${monsterHarm}点伤害`
          );
        }
    ...
    findTreasure() {
      ...
      this.texts.push(
        `你击败${
          this.monster.name
        }后，发现了一个宝箱，打开后获得了<span class="el-tag el-tag--${
          equipItem.quality
        }">${this.$levels[equipItem.quality]}${equipItem.name}(${
          this.$genre[equipItem.type]
        })</span>`
      );
        ...
        if (this.player.level < this.$maxLv) {
          if (this.player.cultivation >= this.player.maxCultivation) {
            ...
            if (
              ...
              this.texts.push(
                `当前境界修为已满, 你需要通过击败<span class="textColor">(${this.player.taskNum} / ${this.player.level})</span>个敌人证道突破`
              );
              return;
            }
            // 清空已击杀敌人
            ...
            this.player.maxCultivation = Math.floor(
              100 * Math.pow(2, this.player.level)
            );
            this.texts.push(
              `恭喜你突破了！当前境界：${this.$levelNames(this.player.level)}`
            );
          } else {
            // 当前修为
      ...
      this.$confirm("", item.name, {
        ...
        message: `<div class="monsterinfo">
                        <div class="monsterinfo-box">
                            ...
                            <p>闪避率: ${
                              (item.dodge * 100).toFixed(2) ?? 0
                            }%</p>
                            <p>暴击率: ${
                              (item.critical * 100).toFixed(2) ?? 0
                            }%</p>
                        </div>
                    </div>`,
      ...
      if (isSuccess) {
        ...
        if (this.player.backpackCapacity > this.player.pets.length) {
          ...
          const health = Math.floor(
            this.getRandomInt(100, 500) * newProperties
          );
          // 闪避
          const dodge = parseFloat(
            this.getRandomFloatInRange(0.001, 0.01) * newProperties
          );
          // 暴击
          ...
          this.player.pets.push({
            ...
            name: item.name,
            level: 1,
            score: equip.calculateEquipmentScore(
              dodge,
              attack,
              health,
              critical,
          ...
          const checkAchievement = (item) => {
            ...
            return (
              ...
              (conditions.dodge === 0 ||
                conditions.dodge <= dodge.toFixed(2)) &&
              (conditions.critical === 0 ||
                conditions.critical <= critical.toFixed(2))
            );
          };
          // 完成成就
          achievement.pet().forEach((item) => {
            ...
            if (
              ...
              !petAchievement.find((i) => i.id === item.id)
            ) {
              // 添加成就
          ...
          this.texts.push(
            `灵宠背包容量已满, 收服${item.name}失败, 转生可增加灵宠背包容量`
          );
        }
        // 恢复回合数
        this.guashaRounds = 10;
