...
const wrapCsvValue = (val: unknown, formatFn?: FormatFn) => {
  let formatted: string = formatFn !== void 0 && formatFn !== null ? formatFn(val) : String(val);

  formatted =
    formatted === void 0 || formatted === null ? '' : formatted;
        ...
        columns
          .value!.map((col: Record<string, unknown>) =>
            wrapCsvValue(
              ...
              (col.format as FormatFn | undefined)
            )
          )
          .join(',')
      )
...
const onNew = (_val: unknown) => {
  void _val;
};

const onView = (_val: unknown) => {
  void _val;
};
</script>
